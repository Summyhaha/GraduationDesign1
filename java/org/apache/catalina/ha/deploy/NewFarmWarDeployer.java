...
import org.apache.catalina.ha.NewClusterDeployer;
 ...
 */
public class NewFarmWarDeployer extends NewClusterListener
        implements NewClusterDeployer, NewFileChangeListener {
    /*--Static Variables----------------------------------------*/
    private static final Log log = LogFactory.getLog(NewFarmWarDeployer.class);
    ...
    protected NewWarWatcher watcher = null;
    ...
    public void start() throws Exception {
        ...
        if (watchEnabled) {
            watcher = new NewWarWatcher(this, getWatchDirFile());
     ...
     */
    @Override
    public void messageReceived(NewClusterMessage msg) {
        try {
    ...
    public void install(String contextName, File webapp) throws IOException {
        ...
        NewFileMessageFactory factory =
            NewFileMessageFactory.getInstance(webapp, false);
        NewFileMessage msg = new NewFileMessage(localMember, webapp.getName(),
                contextName);
        if(log.isDebugEnabled())
    ...
    public void remove(String contextName, boolean undeploy)
        ...
        if (getCluster().getMembers().length > 0) {
            ...
            Member localMember = getCluster().getLocalMember();
            NewUndeployMessage msg = new NewUndeployMessage(localMember, System
