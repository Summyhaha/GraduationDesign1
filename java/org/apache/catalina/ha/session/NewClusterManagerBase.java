...
import org.apache.catalina.ha.tcp.NewReplicationValve;
import org.apache.catalina.session.ManagerBase;
import org.apache.catalina.tribes.io.ReplicationStream;
import org.apache.juli.logging.Log;
import org.apache.juli.logging.LogFactory;

public abstract class NewClusterManagerBase extends ManagerBase implements NewClusterManager {

    private final Log log = LogFactory.getLog(NewClusterManagerBase.class);
     ...
     */
    private volatile NewReplicationValve replicationValve = null ;
    ...
    protected void clone(NewClusterManagerBase copy) {
     ...
     */
    protected void registerSessionAtReplicationValve(NewDeltaSession session) {
        if(replicationValve == null) {
            NewCatalinaCluster cluster = getCluster() ;
            if(cluster != null) {
                Valve[] valves = cluster.getValves();
                if(valves != null && valves.length > 0) {
                    for(int i=0; replicationValve == null && i < valves.length ; i++ ){
                        if(valves[i] instanceof NewReplicationValve) replicationValve =
                                (NewReplicationValve)valves[i] ;
                    }//for
    ...
    protected void startInternal() throws LifecycleException {
        ...
        if (getCluster() == null) {
            ...
            if (cluster instanceof NewCatalinaCluster) {
