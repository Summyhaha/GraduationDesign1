...
import org.apache.catalina.ha.session.NewSerializablePrincipal;
 ...
 */
public class NewClusterSingleSignOn extends SingleSignOn {

    // ----------------------------------------------------- Instance Variables

    protected int messageNumber = 0;

    private NewClusterSingleSignOnListener clusterSSOListener = null;


    // ------------------------------------------------------------- Properties

    private NewCatalinaCluster cluster = null;
    public NewCatalinaCluster getCluster() { return cluster; }
    ...
    protected synchronized void startInternal() throws LifecycleException {
        ...
        clusterSSOListener = new NewClusterSingleSignOnListener(this);
        ...
        try {
            ...
            if(cluster == null) {
                ...
                if(host != null && host instanceof Host) {
                    ...
                    if(cluster != null && cluster instanceof NewCatalinaCluster) {
                        setCluster((NewCatalinaCluster) cluster);
    ...
    protected void associate(String ssoId, Session session) {
        ...
        if (cluster != null && cluster.getMembers().length > 0) {
            messageNumber++;
            NewSingleSignOnMessage msg =
                new NewSingleSignOnMessage(cluster.getLocalMember(),
                                        ssoId, session.getId());
            Manager mgr = session.getManager();
            if ((mgr != null) && (mgr instanceof NewClusterManager)) {
                msg.setContextName(((NewClusterManager) mgr).getName());
            }

            msg.setAction(NewSingleSignOnMessage.ADD_SESSION);

            cluster.send(msg);

            if (containerLog.isDebugEnabled()) {
                containerLog.debug("NewSingleSignOnMessage Send with action "
                                   + msg.getAction());
            }
        }
    ...
    protected void deregister(String ssoId, Session session) {
        ...
        if (cluster != null && cluster.getMembers().length > 0) {
            ...
            msg.setAction(NewSingleSignOnMessage.DEREGISTER_SESSION);
    ...
    protected void deregister(String ssoId) {
        ...
        if (cluster != null && cluster.getMembers().length > 0) {
            ...
            NewSingleSignOnMessage msg =
                new NewSingleSignOnMessage(cluster.getLocalMember(),
                                        ssoId, null);
            msg.setAction(NewSingleSignOnMessage.LOGOUT_SESSION);
    ...
    protected void register(String ssoId, Principal principal, String authType,
        ...
        if (cluster != null && cluster.getMembers().length > 0) {
            ...
            NewSingleSignOnMessage msg =
                new NewSingleSignOnMessage(cluster.getLocalMember(),
                                        ssoId, null);
            msg.setAction(NewSingleSignOnMessage.REGISTER_SESSION);
            msg.setAuthType(authType);
            msg.setUsername(username);
            msg.setPassword(password);

            NewSerializablePrincipal sp = null;
            if (principal instanceof GenericPrincipal) {
                sp = NewSerializablePrincipal.createPrincipal((GenericPrincipal) principal);
                msg.setPrincipal(sp);
            }
    ...
    protected void removeSession(String ssoId, Session session) {
        ...
        if (cluster != null && cluster.getMembers().length > 0) {
            ...
            msg.setAction(NewSingleSignOnMessage.REMOVE_SESSION);
