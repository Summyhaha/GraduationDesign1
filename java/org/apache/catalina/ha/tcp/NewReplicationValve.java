...
import org.apache.catalina.ha.NewClusterSession;
import org.apache.catalina.ha.NewClusterValve;
import org.apache.catalina.ha.session.NewDeltaManager;
import org.apache.catalina.ha.session.NewDeltaSession;
 ...
 */
public class NewReplicationValve
    extends ValveBase implements NewClusterValve {

    private static final org.apache.juli.logging.Log log =
        org.apache.juli.logging.LogFactory.getLog( NewReplicationValve.class );
    ...
    private NewCatalinaCluster cluster = null ;
     ...
     */
    protected final ThreadLocal<ArrayList<NewDeltaSession>> crossContextSessions =
        new ThreadLocal<>() ;
     ...
     */
    public void registerReplicationSession(NewDeltaSession session) {
        List<NewDeltaSession> sessions = crossContextSessions.get();
        if(sessions != null) {
        ...
        throws IOException, ServletException
    {
        ...
        try {
            if(isCrossContext) {
                ...
                crossContextSessions.set(new ArrayList<NewDeltaSession>());
            }
            getNext().invoke(request, response);
            if(context != null && cluster != null
                    && context.getManager() instanceof NewClusterManager) {
            	NewClusterManager clusterManager = (NewClusterManager) context.getManager();
    ...
    protected synchronized void startInternal() throws LifecycleException {
        if (cluster == null) {
            ...
            if (containerCluster instanceof NewCatalinaCluster) {
                setCluster((NewCatalinaCluster)containerCluster);
            } else {
     ...
     */
    protected void sendReplicationMessage(Request request, long totalstart, boolean isCrossContext, NewClusterManager clusterManager) {
        ...
        try {
            ...
            if (!(clusterManager instanceof NewDeltaManager)) {
    ...
    protected void sendCrossContextSession() {
        ...
        if(sessions != null && sessions.size() >0) {
            for(Iterator<NewDeltaSession> iter = sessions.iterator(); iter.hasNext() ;) {
                Session session = iter.next();
                if(log.isDebugEnabled()) {
                    log.debug(sm.getString("ReplicationValve.crossContext.sendDelta",
                            session.getManager().getContext().getName() ));
                }
                sendMessage(session,(NewClusterManager)session.getManager());
                if(doStatistics()) {
    ...
    protected void resetReplicationRequest(Request request, boolean isCrossContext) {
        ...
        if(contextSession instanceof NewDeltaSession){
            resetDeltaRequest(contextSession);
            ((NewDeltaSession)contextSession).setPrimarySession(true);
        }
        if(isCrossContext) {
            List<NewDeltaSession> sessions = crossContextSessions.get();
            if(sessions != null && sessions.size() >0) {
                Iterator<NewDeltaSession> iter = sessions.iterator();
                for(; iter.hasNext() ;) {
                    Session session = iter.next();
                    resetDeltaRequest(session);
                    if(session instanceof NewDeltaSession) {
                        ((NewDeltaSession)contextSession).setPrimarySession(true);
                    }
    ...
    protected void resetDeltaRequest(Session session) {
        ...
        ((NewDeltaSession)session).resetDeltaRequest();
    }
    ...
    */
     ...
     */
    protected void send(NewClusterManager manager, String sessionId) {
    	NewClusterMessage msg = manager.requestCompleted(sessionId);
     ...
     * @param manager
     */
    protected void sendInvalidSessions(NewClusterManager manager) {
    ...
    protected void createPrimaryIndicator(Request request) throws IOException {
        ...
        if ((id != null) && (id.length() > 0)) {
            ...
            if (session instanceof NewClusterSession) {
            	NewClusterSession cses = (NewClusterSession) session;
                if (log.isDebugEnabled()) {
