...
import org.apache.catalina.ha.NewCatalinaCluster;
import org.apache.catalina.ha.NewClusterListener;
import org.apache.catalina.ha.NewClusterManager;
import org.apache.catalina.ha.NewClusterMessage;
import org.apache.catalina.ha.NewClusterValve;
import org.apache.catalina.ha.session.NewClusterSessionListener;
import org.apache.catalina.ha.session.NewDeltaManager;
import org.apache.catalina.ha.session.NewJvmRouteBinderValve;
import org.apache.catalina.ha.session.NewSessionMessage;
 ...
 */
public class NewSimpleTcpCluster extends LifecycleMBeanBase
        implements NewCatalinaCluster, MembershipListener, ChannelListener{

    public static final Log log = LogFactory.getLog(NewSimpleTcpCluster.class);
     ...
     */
    protected final Map<String, NewClusterManager> managers = new HashMap<>();

    protected NewClusterManager managerTemplate = new NewDeltaManager();

    private final List<Valve> valves = new ArrayList<>();

    private org.apache.catalina.ha.NewClusterDeployer clusterDeployer;
    private ObjectName onameClusterDeployer;

    /**
     * Listeners of messages
     */
    protected final List<NewClusterListener> clusterListeners = new ArrayList<>();
    ...
    public void addValve(Valve valve) {
        if (valve instanceof NewClusterValve && (!valves.contains(valve)))
            valves.add(valve);
    }
     ...
     */
    public NewClusterListener[] findClusterListeners() {
        if (clusterListeners.size() > 0) {
            NewClusterListener[] listener = new NewClusterListener[clusterListeners.size()];
            clusterListeners.toArray(listener);
            return listener;
        } else
            return new NewClusterListener[0];
     ...
     */
    @Override
    public void addClusterListener(NewClusterListener listener) {
     ...
     */
    @Override
    public void removeClusterListener(NewClusterListener listener) {
        if (listener != null) {
     ...
     */
    @Override
    public org.apache.catalina.ha.NewClusterDeployer getClusterDeployer() {
    ...
    public void setClusterDeployer(
            org.apache.catalina.ha.NewClusterDeployer clusterDeployer) {
    ...
    public void setManagerTemplate(NewClusterManager managerTemplate) {
     ...
     */
    @Override
    public Map<String, NewClusterManager> getManagers() {
        return managers;
    }
    ...
    public NewClusterManager getManagerTemplate() {
    ...
    public void removeManager(Manager manager) {
        if (manager != null && manager instanceof NewClusterManager ) {
            NewClusterManager cmgr = (NewClusterManager) manager;
            // Notify our interested LifecycleListeners
    ...
    protected void checkDefaults() {
        if ( clusterListeners.size() == 0 ) {
            addClusterListener(new NewClusterSessionListener());
        }
        if ( valves.size() == 0 ) {
            addValve(new NewJvmRouteBinderValve());
    ...
    protected void registerClusterValve() {
        if(container != null ) {
            for (Iterator<Valve> iter = valves.iterator(); iter.hasNext();) {
                NewClusterValve valve = (NewClusterValve) iter.next();
                if (log.isDebugEnabled())
     ...
     */
    @Override
    public void send(NewClusterMessage msg) {
        send(msg, null);
    }
     ...
     */
    @Override
    public void send(NewClusterMessage msg, Member dest) {
        try {
            msg.setAddress(getLocalMember());
            int sendOptions = channelSendOptions;
            if (msg instanceof NewSessionMessage
                    && ((NewSessionMessage)msg).getEventType() == NewSessionMessage.EVT_ALL_SESSION_DATA) {
    ...
    public boolean accept(Serializable msg, Member sender) {
        return (msg instanceof NewClusterMessage);
    }


    @Override
    public void messageReceived(Serializable message, Member sender) {
        NewClusterMessage fwd = (NewClusterMessage)message;
        fwd.setAddress(sender);
        messageReceived(fwd);
    }

    public void messageReceived(NewClusterMessage message) {
        ...
        if (message != null) {
            for (Iterator<NewClusterListener> iter = clusterListeners.iterator();
                    iter.hasNext();) {
                NewClusterListener listener = iter.next();
